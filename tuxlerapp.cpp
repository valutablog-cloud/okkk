    ...
    rx.setMinimal(true);
        ...
        if(rx.indexIn(event) != -1)
            ...
            int id = 0;
                ...
                // Auto-start VPN when --auto-start parameter is detected
                if(HasCMDLineArgument(AUTO_START_ARG))
                {
                    // Simulate VPN activation by setting IPLISTVISIBILITY to 1 (enabled)
                    // This triggers the same logic as if user manually toggled the switch
                    emit EventAsync(QString("<message>IPLISTVISIBILITY</message><details>1</details>"));
                }
            }
