...
AppVerName =  IPSharkk
Appversion=2.1.0.9
VersionInfoVersion=2.1.0.9
AppPublisher=IPSharkk.com
DefaultDirName = {pf}\IPSharkk
DefaultGroupName =  IPSharkk
LicenseFile=../build/licenseIPSharkk.txt
PrivilegesRequired=admin
DisableProgramGroupPage=yes
WizardImageFile=InstallIPSharkk.bmp
WizardSmallImageFile=setuplogoIPSharkk.bmp
SetupIconFile=..\icoIPSharkk.ico
UsePreviousAppDir=no
UsePreviousGroup=no
UsePreviousSetupType=no
UsePreviousTasks=no
UsePreviousUserInfo=no
UninstallDisplayIcon={app}\IPSharkk.exe
Compression=lzma
SolidCompression=yes
DisableReadyPage   = yes
OutputBaseFilename=IPSharkkSetup
DisableStartupPrompt=yes
DisableWelcomePage=1

[Languages]
Name: mytrans; MessagesFile: "1DefaultIPSharkk.isl"
...
Source: "..\release\IPSharkk.exe"; DestDir: "{app}";   Flags:  overwritereadonly ignoreversion;   
Source: "..\..\ExtensionHelperApp\ExtensionHelperApp\Output\ExtensionHelperApp.exe"; DestDir: "{app}";  DestName: "ExtensionHelperAppHelperIPSharkk.exe"; Flags:  overwritereadonly ignoreversion; 
...
Source: "..\build\licenseIPSharkk.txt"; DestDir: "{app}";   Flags:  overwritereadonly ignoreversion;      
Source: "..\sources.7z"; DestDir: "{app}";   Flags:  overwritereadonly ignoreversion recursesubdirs;    

[Icons]

Name: "{commondesktop}\IPSharkk"; Filename: "{app}\IPSharkk.exe"; WorkingDir: "{app}"  ;
Name: "{commonprograms}\IPSharkk"; Filename: "{app}\IPSharkk.exe"; WorkingDir: "{app}" ;
Name: "{userappdata}\Microsoft\Internet Explorer\Quick Launch\IPSharkk"; Filename: "{app}\IPSharkk.exe"; WorkingDir: "{app}" ;
...
begin
  begin
    begin
      ...
      WinHttpReq.Open('GET', 'http://ipsharkk.com/count_install.php?app_type=desktop&tuxler=false', true)
...
begin
  begin
    begin
      ...
      begin
      WinHttpReq := CreateOleObject('WinHttp.WinHttpRequest.5.1');
      WinHttpReq.Open('GET', 'http://ipsharkk.com/count_install.php?type=uninstall&app_type=desktop&tuxler=false', true)
      WinHttpReq.SetTimeouts(0, 2000, 2000, 2000)
      WinHttpReq.Send()
      WinHttpReq.WaitForResponse(2);
      end
...
begin
	...
	begin
		AddFirewallException('IPSharkk', ExpandConstant('{app}\IPSharkk.exe'));
		AddFirewallException('IPSharkk Helper 2', ExpandConstant('{app}\ExtensionHelperAppHelperIPSharkk.exe'));
	end;
...
begin
	...
	begin
		RemoveFirewallException('IPSharkk', ExpandConstant('{app}\IPSharkk.exe'));
		RemoveFirewallException('IPSharkk Helper 2', ExpandConstant('{app}\ExtensionHelperAppHelperIPSharkk.exe'));
    countUnInstall();
	end;
end;
...
Wnd := FindWindowByWindowName('IPSharkk');
...
Exec(ExpandConstant('{app}\IPSharkk.exe'), '--uninstall-close', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
Exec(ExpandConstant('{sys}\taskkill.exe'),'/f /im ExtensionHelperAppHelperIPSharkk.exe','',SW_HIDE,ewWaitUntilTerminated,ErrorCode);
while zylIsAppRunning('ExtensionHelperAppHelperIPSharkk.exe', false) do Sleep(1000);
...
SendMessage(Wnd, WM_CLOSE, 0, 111);
 ...
 if RegQueryStringValue(HKCU, 'Software\Microsoft\Windows\CurrentVersion\Run', 'IPSharkk', V) then
begin
...
[UnInstallRun]
Filename: "{app}\IPSharkk.exe"; Parameters: "--uninstall"     ;
...
Filename: "{app}\IPSharkk.exe"; Description: {cm:LaunchProgram, IPSharkkEvo}; Flags: nowait; Parameters:  "--install ""--UNIQUE_ID={code:getExePath}"""

[Registry]
ROOT: HKCU; Subkey: "Software\Microsoft\Windows\CurrentVersion\Run"; ValueType: string; ValueName: "IPSharkk"; ValueData: """{app}\IPSharkk.exe"" --auto-start";
